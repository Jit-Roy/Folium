...
from PySide6.QtCore import Qt, QRectF, QSize, QTimer, Signal, QDate
...
from core.models import WeeklyGoalEntry, WeeklyPlan, WeeklyPlanNote
...
class _CircleCheck(QWidget):
    ...
    def paintEvent(self, _event) -> None:
        ...
        if self._checked:
            ...
            # Border-only circle
            ring_color = accent if self._is_today else QColor("#B48EAD" if self._hovered else "#444444")
            if self._hovered:
                ring_color = accent
...
class _GoalRow(QWidget):
    ...
    def _create_icon(self, kind: str) -> QIcon:
        ...
        pen = QPen(QColor("#B48EAD" if self._hovered else _TEXT_PRI))
        pen.setWidth(2)
        painter.setPen(pen)
...
class PlannerService:
    def get_or_create_weekly_plan(self, week_start_date):
        ws_str = str(week_start_date)
        with get_session() as session:
            plan = session.query(WeeklyPlan).filter_by(week_start_date=ws_str).first()
            if not plan:
                plan = WeeklyPlan(week_start_date=ws_str, created_date=str(date.today()), title='Weekly Goals')
                session.add(plan)
                session.commit()
                session.refresh(plan)
            # attach entries
            plan.entries = session.query(WeeklyGoalEntry).filter_by(plan_id=plan.id).all()
            return plan

    def get_weekly_plan_notes(self, plan_id):
        with get_session() as session:
            notes = session.query(WeeklyPlanNote).filter_by(plan_id=plan_id).all()
            return {n.day_of_week: n.note for n in notes}

    def save_weekly_plan_note(self, plan_id, day_of_week, note_text):
        with get_session() as session:
            note = session.query(WeeklyPlanNote).filter_by(plan_id=plan_id, day_of_week=day_of_week).first()
            if note:
                note.note = note_text
            else:
                session.add(WeeklyPlanNote(plan_id=plan_id, day_of_week=day_of_week, note=note_text))
            session.commit()

    def create_weekly_goal_entry(self, plan_id, entry):
        with get_session() as session:
    ...
    def update_weekly_goal_entry(self, entry_id, entry):
        with get_session() as session:
            db_entry = session.query(WeeklyGoalEntry).get(entry_id)
            if db_entry:
                db_entry.title = entry.title
                db_entry.completed = entry.completed
                session.commit()

    def delete_weekly_goal_entry(self, entry_id):
        with get_session() as session:
            db_entry = session.query(WeeklyGoalEntry).get(entry_id)
            if db_entry:
    ...
    def get_weekly_goal_entry(self, entry_id):
        with get_session() as session:
            return session.query(WeeklyGoalEntry).get(entry_id)


class PlannerPage(QWidget):
    week_changed = Signal(QDate)
    ...
    def _render(self) -> None:
        ...
        total_goals = len(plan.entries)
        completed_goals = sum(1 for e in plan.entries if e.completed)
        self.stats_updated.emit(completed_goals, total_goals)
        
        start_date = days[0][2]
        self.week_changed.emit(QDate(start_date.year, start_date.month, start_date.day))
    ...
    def jump_to_date(self, qdate: QDate) -> None:
        target = qdate.toPython()
        today = date.today()
        target_monday = target - timedelta(days=target.weekday())
        today_monday = today - timedelta(days=today.weekday())
        self.week_offset = (target_monday - today_monday).days // 7
        self._render()
