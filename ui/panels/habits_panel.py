...
from core.models import HabitCompletion
    ...
    def __init__(self, days: list[tuple[str, str, date]],
        ...
        for i, (abbr, num, _d) in enumerate(days):
            ...
            abbr_lbl.setStyleSheet(
                f"color: {'#B48EAD' if is_t else _T_SEC};"
            ...
            num_lbl.setStyleSheet(
                f"color: {'#B48EAD' if is_t else _T_TER};"
                f"font-size: 9.5pt;"
                f"font-weight: {'600' if is_t else '400'};"
                "background: transparent;"
            )
...
class HabitRepoMock:
    def mark_completed(self, habit_id, date_val):
        date_str = str(date_val)
        with get_session() as session:
            existing = session.query(HabitCompletion).filter_by(habit_id=habit_id, completion_date=date_str).first()
            if not existing:
                session.add(HabitCompletion(habit_id=habit_id, completion_date=date_str))
                session.commit()

    def get_completions(self, habit_id, start_date=None, end_date=None):
        with get_session() as session:
            logs = session.query(HabitCompletion).filter_by(habit_id=habit_id).all()
            # Try parsing the dates
            dates = []
            from datetime import datetime
            for log in logs:
                try:
                    dates.append(datetime.strptime(log.completion_date, '%Y-%m-%d').date())
                except:
                    pass
            return set(dates)

    def unmark_completed(self, habit_id, date_val):
        date_str = str(date_val)
        with get_session() as session:
            session.query(HabitCompletion).filter_by(habit_id=habit_id, completion_date=date_str).delete()
            session.commit()
    ...
    def get(self, habit_id):
        with get_session() as session:
            return session.query(Habit).get(habit_id)

    def get_stats(self, week_start, week_end):
        ...
        with get_session() as session:
            ...
            if total_habits == 0:
                return {"completion_rate": 0, "perfect_days": 0, "longest_streak_name": "None", "longest_streak": 0}
            ...
            for c in completions:
                try:
                    c_date = datetime.strptime(c.completion_date, '%Y-%m-%d').date()
                    if week_start <= c_date <= week_end:
                        week_completions += 1
                except:
                    pass
            completion_rate = min(100, int((week_completions / (total_habits * 7)) * 100))
            
            # 2. Perfect Days (Current Month)
            today = date.today()
            ...
            for c in completions:
                try:
                    c_date = datetime.strptime(c.completion_date, '%Y-%m-%d').date()
                    if first_day_of_month <= c_date <= today:
                        date_counts[c_date] = date_counts.get(c_date, 0) + 1
                except:
                    pass
            perfect_days = sum(1 for d, count in date_counts.items() if count >= total_habits)
            
            # 3. Longest Streak
            habit_dates = {h.id: set() for h in habits}
            for c in completions:
                try:
                    c_date = datetime.strptime(c.completion_date, '%Y-%m-%d').date()
                    if c.habit_id in habit_dates:
                        habit_dates[c.habit_id].add(c_date)
                except:
                    pass
            
            max_streak = 0
            max_streak_name = "None"
            
            for h in habits:
                dates = sorted(list(habit_dates[h.id]))
                if not dates:
                    continue
                current_streak = 1
                local_max = 1
                ...
                if local_max > max_streak:
                    ...
                    max_streak_name = h.title
                    
            return {
                "completion_rate": completion_rate,
                "perfect_days": perfect_days,
                "longest_streak_name": max_streak_name,
                "longest_streak": max_streak
            }

class HabitService:
    def __init__(self):
        self.habits = HabitRepoMock()

    def create_habit(self, habit):
        with get_session() as session:
            if type(habit.created_date) != str:
    ...
    def update_habit(self, habit_id, habit):
        with get_session() as session:
            h = session.query(Habit).get(habit_id)
            if h:
                h.title = habit.title
    ...
    def delete_habit(self, habit_id):
        with get_session() as session:
            h = session.query(Habit).get(habit_id)
            if h:
                session.query(HabitCompletion).filter_by(habit_id=habit_id).delete()
                session.delete(h)
                session.commit()
