...
from .models import JournalEntry, Habit, Note, NoteNode, NoteNotebook, NoteSection, Project, ProjectIdea, Task, TaskStatus, WeeklyPlan, WeeklyGoalEntry
...
class DiaryRepository:
    ...
    def get_by_date(self, entry_date: date) -> JournalEntry | None:
        with self.db.session() as conn:
            row = conn.execute(
    ...
    def update(self, entry_id: int, entry: JournalEntry) -> None:
        with self.db.session() as conn:
            conn.execute(
