...
from .models import JournalEntry, Habit, Course, CourseMilestone, Task, TaskStatus, WeeklyGoalEntry, WeeklyPlan
...
class ProductivityService:
    ...
    def get_diary_entry(self, entry_date: date) -> JournalEntry | None:
        return self.diary.get_by_date(entry_date)

    def list_diary_entries(self) -> list[JournalEntry]:
        return self.diary.list()

    def create_diary_entry(self, entry: JournalEntry) -> int:
        return self.diary.add(entry)

    def update_diary_entry(self, entry: JournalEntry) -> None:
        if entry.id is None:
            return
